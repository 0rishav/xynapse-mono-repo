...
export const updateMCQQuestion = CatchAsyncError(async (req, res, next) => {
  ...
  // Direct destructuring from req.body
  const {
    title,
    description,
    difficulty,
  ...
  // Validation step (Sanitization hatane ke baad direct body pass kar rahe hain)
  const validationError = validateMCQData(req.body, true);
  if (validationError) {
  ...
  // Basic fields update using Nullish Coalescing (??)
  mcq.title = title ?? mcq.title;
  mcq.description = description ?? mcq.description;
  mcq.difficulty = difficulty?.toLowerCase() ?? mcq.difficulty;
  mcq.accessLevel = accessLevel?.toLowerCase() ?? mcq.accessLevel;
  mcq.explanation = explanation ?? mcq.explanation;

  // Complex fields (Arrays/Objects) logic
  ...
  if (typeof multipleCorrect === "boolean") mcq.multipleCorrect = multipleCorrect;
  ...
  if (chapterId !== undefined) mcq.chapterId = chapterId || undefined;
  if (paperId !== undefined) mcq.paperId = paperId || undefined;
